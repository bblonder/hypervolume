hypervolume_gaussian_old <- function(data, name=NULL, verbose=TRUE, output.density=10^(ncol(data)), expectation.num.shifts=2, expectation.bin.widths=2*estimate_bandwidth(data), kde.bandwidth=estimate_bandwidth(data)/2, kde.chunksize=1e4, threshold=estimate_threshold_gaussian(sd.count=expectation.num.shifts, bandwidth=kde.bandwidth))
{
  data <- as.matrix(data)
