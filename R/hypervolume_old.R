hypervolume_old <- function(data, repsperpoint=NULL, bandwidth, quantile=0.0, name=NULL, verbose=TRUE, warnings=TRUE)
{
  if (warnings==TRUE)
  {
    warning('This function is deprecated. Consider porting code to new hypervolume_box() function ')
  }
  ...
  # give the inherited class definition
  result = new("HypervolumeOld", Name=ifelse(is.null(name), "untitled", toString(name)))
