occupancy_filter <- function(hv, operator = NULL, filter = NULL, tol = 1e-10){
  ...
  if(inherits(hv, "Hypervolume")){
    if(! any(hv@Method %in% c("n_occupancy", "n_occupancy_test")) ){
      stop("Construction method must be one of n_occupancy or n_occupancy_test")
    }
    i_vol <- get_volume(hv)
    i_vol_length <- length(hv@ValueAtRandomPoints)
  ...
  if(inherits(hv, "HypervolumeList")){
    ...
    if(any(! hyper_methods %in% c("n_occupancy", "n_occupancy_test"))){
      stop("Construction method must be one of n_occupancy or n_occupancy_test")
    }
    ...
    vols <- get_volume(hv)
    all_length <- length(hv@HVList[[1]]@ValueAtRandomPoints)
    i_length <- unlist(lapply(hv@HVList, function(z) sum(z@ValueAtRandomPoints != 0)))
