plot_n_occupancy <- function(x, show.3d=FALSE,plot.3d.axes.id=NULL,
                             ...
                             cex.random=0.75,cex.data=0.75,cex.axis=0.75,cex.names=1.0,cex.legend=0.75,
  ...
  #### ALEX !!!!!! 
  # transform Hypervolume into HypervolumeList, as done by default in the hypervolume package
  
  if(identical(class(x)[1], "Hypervolume")){
    ...
    if (is.null(all))
    {
      ...
      {
        all <- matrix(0,ncol=2+alldims,nrow=1,dimnames=list(NULL,c(paste("X",1:alldims,sep=""),"ID","Density","Occupancy")))
      }
      else
      {
        all <- matrix(0,ncol=2+alldims,nrow=1,dimnames=list(NULL,c(dimnames(x@HVList[[1]]@RandomPoints)[[2]],"ID","Density","Occupancy")))
      }
    ...
    {
      varlimlist = vector('list',ncol(all)-3)
    ...
    if (show.3d==FALSE)
    {
      ...
      par(mfrow=c(ncol(all)-3, ncol(all)-3))
      par(mar=c(0,0,0,0))
      par(oma=c(0.5,0.5,0.5,0.5))
      
      for (i in 1:(ncol(all)-3))
      {
        for (j in 1:(ncol(all)-3))  
        {
          if (j > i)
          {
