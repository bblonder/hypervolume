hypervolume_resample <- function(name, hv, method, n = 10, points_per_resample = 'sample_size', seq = 3:nrow(hv@Data), k = 5, cores = 1, verbose = TRUE, mu = NULL, sigma = NULL, cols_to_weigh = 1:ncol(hv@Data), weight_func = NULL) {
  ...
  } else if (method == 'weighted bootstrap' & (length(mu) != length(sigma) | length(mu) != length(cols_to_weigh))) {
  ...
  if (method == 'bootstrap') {
    ...
    return(weighted_bootstrap(name, hv, n, points_per_resample, cores, verbose, mu, sigma, cols_to_weigh))
