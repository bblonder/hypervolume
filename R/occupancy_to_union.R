occupancy_to_union <- function(hv_list, method = "all", m = 2, tol = 1e-10){
  ...
  if(inherits(hv_list, "HypervolumeList")){
    ...
    if(any(! hyper_methods %in% c("n_occupancy", "n_occupancy_test", "occupancy_to_intersection", "occupancy_to_unshared"))){
      stop("Construction method must be n_occupancy, n_occupancy_test, occupancy_to_intersection or occupancy_to_unshared")
    }
    ...
    Contact the maintener if none of the above applies to you.")
      ...
      temp_hyper@Method <- "occupancy_to_union"
      temp_hyper@Name <- "union"
